...
# Definition of outer Schur complement for the order ((u,p),(B,E,j))
class SchurPCBEj(AuxiliaryOperatorPC):
    def form(self, pc, V, U):
        [B, B3, E, E3, j, j3] = split(U)
        [C, C3, Ff, Ff3, kk, kk3] = split(V)
        state = self.get_appctx(pc)['state']
        [u_n, u3_n, p_n, B_n, B3_n, E_n, E3_n, j_n, j3_n] = split(state)

        A = (
            + inner(j, Ff) * dx
            - inner(B3, scurl(Ff)) * dx
            + inner(j3, Ff3) * dx
            - inner(B, vcurl(Ff3)) * dx
            + inner(vcurl(E3), C) * dx
            + inner(scurl(E), C3) * dx
            + 1/Rem * inner(j, kk) * dx
            - inner(E, kk) * dx
            - inner(vcross(u_n, B3), kk) * dx
            + inner(vcross(B, u3_n), kk) * dx
            + RH * inner(vcross(j_n, B3), kk) * dx
            + RH * inner(vcross(j, B3_n), kk) * dx
            - RH * inner(vcross(B_n, j3), kk) * dx
            - RH * inner(vcross(B, j3_n), kk) * dx
            + 1/Rem * inner(j3, kk3) * dx
            - inner(E3, kk3) * dx
            - inner(scross(u_n, B), kk3) * dx
            + RH * inner(scross(j_n, B), kk3) * dx
            + RH * inner(scross(j, B_n), kk3) * dx
        )
        ...
        bcs = [DirichletBC(V.function_space().sub(0), 0, "on_boundary"),
               ...
               DirichletBC(V.function_space().sub(4), 0, "on_boundary"),
               DirichletBC(V.function_space().sub(5), 0, "on_boundary"),
               ]
        
        A = inner(B, C) * dx + inner(B3, C3) * dx + dt_factor*dt*A
        return (A, bcs)


class SchurPCB3Ej(AuxiliaryOperatorPC):
    def form(self, pc, V, U):
        [B3, E, j] = split(U)
        [C3, Ff, kk] = split(V)
        state = self.get_appctx(pc)['state']
...
#            + RH * inner(scross(j, B_n), kk3) * dx
        ...
        A = inner(B3, C3) * dx + dt_factor*dt*A
        return (A, bcs)
...
#         "mat_mumps_icntl_14": ICNTL_14,
         ...
         "mg_levels_ksp_norm_type": "unpreconditioned",
         "mg_levels_pc_type": "python",
#         "mg_levels_pc_factor_mat_solver_type": "mumps",
#         "mg_levels_mat_mumps_icntl_14": ICNTL_14,
#         "mg_levels_pc_python_type": "firedrake.ASMStarPC",
#         "mg_levels_pc_star_backend": "tinyasm",
         "mg_levels_pc_python_type": "firedrake.PatchPC",
         "mg_levels_patch_pc_patch_save_operators": True,
...
#         "mg_coarse_mat_mumps_icntl_14": ICNTL_14,
         "mg_coarse_pc_python_type": "firedrake.AssembledPC",
         "mg_coarse_assembled": {
               "mat_type": "aij",
               "pc_type": "telescope",
...
outerschurlulu = {
    ...
    "aux": {
        ...
        "pc_fieldsplit_schur_factorization_type": "full",
        "pc_fieldsplit_schur_precondition": "user",
        "pc_fieldsplit_0_fields": "0,3,5",
        "pc_fieldsplit_1_fields": "1,2,4",
        "fieldsplit_0": {
            "ksp_type": "gmres",
            "pc_type": "lu",
            "pc_factor_mat_solver_type": "mumps",
        ...
        "fieldsplit_1": {
            ...
            "ksp_max_it": 2,
            "pc_type": "python",
            "pc_python_type": "__main__.SchurPCB3Ej",
            "aux_pc_type": "lu",
            "aux_pc_factor_mat_solver_type": "mumps",
            "aux_mat_mumps_icntl_14": ICNTL_14,
            }
...
fs2by2 = {
    ...
    "pc_fieldsplit_1_fields": "3,4,5,6,7,8",
}
...
fs2by2sstarlu = {
    ...
    "pc_fieldsplit_0_fields": "0,1,2",
    "pc_fieldsplit_1_fields": "3,4,5,6,7,8",
    "fieldsplit_0": nsfsstar,
    "fieldsplit_1": outerschurstarlu,
}
...
solvers = {"lu": lu, "fs2by2": fs2by2, "fs2by2nslu": fs2by2nslu, "fs2by2slu": fs2by2slu, "fs2by2lu": fs2by2lu, "fs2by2sstarlu": fs2by2sstarlu}
...
parser.add_argument("--RH", nargs='+', type=float, default=[1])
...
V3 = FunctionSpace(mesh, "CG", k)
Q = FunctionSpace(mesh, "DG", k-1)  # p
Eel = FiniteElement("N1curl", mesh.ufl_cell(), k, variant="integral") 
EE = FunctionSpace(mesh, Eel)
EE3 = FunctionSpace(mesh, "CG", k)
Bel = FiniteElement("N1div", mesh.ufl_cell(), k, variant="integral")
BB = FunctionSpace(mesh, Bel)
BB3 = FunctionSpace(mesh, "CG", k)
Jel = FiniteElement("N1curl", mesh.ufl_cell(), k, variant="integral")
JJ = FunctionSpace(mesh, Jel)  # j
JJ3 = FunctionSpace(mesh, "CG", k)

Z = MixedFunctionSpace([V, V3, Q, BB, BB3, EE, EE3, JJ, JJ3])
...
if testproblem == "ldc":
    ...
    u3_ex = Constant(0, domain=mesh)
    p_ex = ((1-myk*myk)/2)*(1 + 1/(b*b))
    B_ex = as_vector([sinh(2*pi*y)/b, myk*sin(2*pi*x)/b])
    B3_ex = Constant(0, domain=mesh)
#    E_ex = Constant(0, domain=mesh)
    j_ex = Constant([0, 0], domain=mesh)
    E_ex = Constant([0, 0], domain=mesh)
    j3_ex = scurl(B_ex)
    E3_ex = 1/Rem * j3_ex - scross(u_ex, B_ex) + RH*scross(j_ex, B_ex)
    ...
    bcs = [DirichletBC(Z.sub(0), u_ex, bcs_ids_apply),  # 4 == upper boundary (y==1)
           DirichletBC(Z.sub(1), u3_ex, "on_boundary"),
           PressureFixBC(Z.sub(2), 0, 1),
           DirichletBC(Z.sub(3), B_ex, "on_boundary"),
           DirichletBC(Z.sub(4), B3_ex, "on_boundary"),
           DirichletBC(Z.sub(5), E_ex, "on_boundary"),
           DirichletBC(Z.sub(6), E3_ex, "on_boundary"),
           DirichletBC(Z.sub(7), j_ex, "on_boundary"),
           DirichletBC(Z.sub(8), j3_ex, "on_boundary"),
        ]
    ...
    B_rhs = interpolate(B_rhs, Z.sub(3))
...
def form(z, test_z, Z, timelevel):
    (u, u3, p, B, B3, E, E3, j, j3) = split(z)
    (v, v3, q, C, C3, Ff, Ff3, kk, kk3) = split(test_z)
    eps = lambda x: sym(grad(x))
    F = (
        + 2/Re * inner(eps(u), eps(v))*dx
        + 2/Re * inner(grad(u3), grad(v3))*dx
        # + advect * inner(dot(grad(u), u), v) * dx
        + gamma * inner(div(u), div(v)) * dx
        - S * inner(vcross(j, B3), v) * dx
        + S * inner(vcross(B, j3), v) * dx
        - S * inner(scross(j, B), v3) * dx
        - inner(p, div(v)) * dx
        - inner(div(u), q) * dx
        + inner(j, Ff) * dx
        - inner(B3, scurl(Ff)) * dx
        ...
        + 1/Rem * inner(j, kk) * dx
        - inner(E, kk) * dx
        - inner(vcross(u, B3), kk) * dx
        + inner(vcross(B, u3), kk) * dx
        + RH * inner(vcross(j, B3), kk) * dx
        - RH * inner(vcross(B, j3), kk) * dx
        + 1/Rem * inner(j3, kk3) * dx
        - inner(E3, kk3) * dx
        - inner(scross(u, B), kk3) * dx
        + RH * inner(scross(j, B), kk3) * dx
    )
    ...
    F += advect * inner(dot(grad(u3), u), v3) * dx  
...
def J_form(F, z, test_z):
    ...
    (v, v3, q, C, C3, Ff, Ff3, kk, kk3) = split(test_z)
    w = TrialFunction(Z)
    [w_u, w_u3, w_p, w_B, w_B3, w_E, w_E3, w_j, w_j3] = split(w)
    ...
    if linearisation == "newton":
        ...
        J_picard = (
              J_newton
            - dt_factor * dt * S * inner(vcross(w_B, E3), v) * dx  # J_tilde
...
def initial_condition():
    ...
    z.sub(1).interpolate(u3_ex)
    z.sub(2).interpolate(p_ex)
    if testproblem == "island_coal":
        z.sub(3).interpolate(B_ex + B_pert)
    else:    
        z.sub(3).interpolate(B_ex)
    z.sub(4).interpolate(B3_ex)
    z.sub(5).interpolate(E_ex)
    z.sub(6).interpolate(E3_ex)
    z.sub(7).interpolate(j_ex)
    z.sub(8).interpolate(j3_ex)
    return z
...
F_cn = (
   ...
   - inner(split(z0)[4], split(z_test)[4])*dx
...
F_bdf2 = (
   ...
   + inner(split(z)[4], split(z_test)[4])*dx
   - 4.0/3.0*inner(split(z0)[4], split(z_test)[4])*dx
   + 1.0/3.0*inner(split(z1)[4], split(z_test)[4])*dx
...
# Recon Rate = 1/Rm * (j(X) - j0(X))  where X is the X-point
# So compute J at origin at time 0
#(u00,p00,B00,E00) = u_ex, p_ex, B_ex, E_ex
#z00 = Function(Z)
#z00.sub(2).interpolate(B00)
#j00CG = project(scurl(z00.sub(2)),JspaceCG)
(u_,u3_, p_, B_, B3_, E_, E3_, j_, j3_) = z.split()
...
#outputdir = "/home/laakmann/hartmann/testdir3/island_output/"
#reconrateCG = open(outputdir + "reconrate_cg.txt", "w")
solver_BCG.solve()
j00CG = j_CG((0,0))
...
filename = "reconrate_{0:}_{1:}_{2:1.7f}_{3:}x{4:}.txt".format(int(float(Rem)),int(float(Re)), float(RH),Nx, Nx)
if restart:
    reconrateCG = open(outputdir + filename, "a+")
else:
...
if args.solver_type in ["fs2by2", "fs2by2nslu"]:
    params["fieldsplit_1"] = outerschurstar if hierarchy == "uniform" else outerschurmstarmstar
...
def run(re, rem, s):
    ...
    if restart:
        ...
        with open(outputdir + filename, "r") as f:
                ...
                last_line = lines[-1]
                t.assign(float(last_line.split('&')[0]))
                ntimestep = int(len(lines))
    total_nonlinear_its = 0  # number of total nonlinear iterations
    total_linear_its = 0  # number of total linear iterations

    if restart:
        chk = DumbCheckpoint("dump/"+str(float(ind2))+str(float(RH))+str(float(dt))+str(linearisation)+str(testproblem), mode=FILE_READ)
        chk.load(z0)
        chk.load(z1)
        chk.load(z)
    ...
    while (float(t) < float(T-dt)+1.0e-10):
        ...
        if mesh.comm.rank == 0:
            print(GREEN % ("Solving for #dofs = %s, Re = %s, Rem = %s, gamma = %s, gamma2 = %s, S = %s, RH = %s, baseN = %s, nref = %s, "
                           "linearisation = %s, testproblem = %s, discr = %s, k = %s"
                           % (Z.dim(), float(re), float(rem), float(gamma), float(gamma2), float(S), float(RH), int(baseN), int(nref),
...
#            solver_ie.solve()
        ...
        if solution_known:
            ...
            if output:
                pvd.write(u, u3, p, B, B3, E, E3, j, j3, time=float(t))
        ...
        (u_, u3_, p_, B_, B3_, E_, E3_, j3, j3_) = z.split()
        ...
        chk = DumbCheckpoint("dump/"+str(float(ind2))+str(float(RH))+str(float(dt))+str(linearisation)+str(testproblem), mode=FILE_CREATE)
        chk.store(z0)
        chk.store(z1)
        chk.store(z)
