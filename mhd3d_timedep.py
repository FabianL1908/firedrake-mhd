...
class SchurPCup(AuxiliaryOperatorPC):
    def form(self, pc, V, U):
        ...
        A = (
              ...
              - inner(p, div(v)) * dx
              - inner(div(u), q) * dx
              + alpha*S * inner(cross(B_n, cross(u, B_n)), v) * dx
        )
...
parser.add_argument("--testproblem", choices=["ldc"], default="ldc")
...
sigma = Constant(1) * Z.sub(0).ufl_element().degree()**2
...
def J_form(F, z, test_z):
    ...
    if linearisation == "newton":
        ...
        J_mdp = (
              J_newton
            - dt_factor * dt * inner(cross(w_u, B), Ff) * dx  # G
                    )
        J = J_mdp
        ...
        J_picard = (
              J_newton
            - dt_factor * dt * S * inner(cross(w_B, E), v) * dx  # J_tilde
            - dt_factor * dt * S * inner(cross(w_B, cross(u, B)), v) * dx  # D_1_tilde
            - dt_factor * dt * S * inner(cross(B, cross(u, w_B)), v) * dx  # D_2_tilde
            - dt_factor * dt * inner(cross(u, w_B), Ff) * dx  # G_tilde
                    )
        J = J_picard
